...
void GameLogic::add_soldier(int* player_id) {
    ...
    int16_t pos_x = std::rand() % 10 + 1;
    while (!game_map.check_free_position(pos_x,5)){
        int16_t pos_x = std::rand() % 10 + 1;
    }
    Soldier* soldier = new Soldier(idf,game_map,pos_x,5);
    game_map.add_soldier(soldier,pos_x,5);
